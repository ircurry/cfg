...
in {
  config = lib.mkIf (cfg == "rofi-logout") {
    home.packages = [ pkgs.nerdfonts rofi-logout ];
    ...
    assertions = [{
      assertion = (menu == "rofi-wayland")
        || (config.nocturne.graphical.rofi.enable);
      message =
        "rofi-logout is the logout menu but rofi is not installed. Please enable rofi or set it as the default menu.";
    }];
  };
}
