...
#+begin_src emacs-lisp
  ...
  where the first element is a symbol and the second is a string that is the hex
  value of the color.  The names should corespond to the default-colors schema.
  For the default schema definition please consult the doc string of
  `cur-theme--check-default-schema'.  CUSTOM-COLORS is the same as DEFAULT-COLORS
  but with no schema.  FACES is a list of faces in the form (FACE PROPERTIES).
  For example, (bold :background bg :foreground fg :weight 'bold)."
    (declare (indent 1))
    `(let* (,@default-colors
            ,@custom-colors)
       (deftheme ',theme-name
...
** Gruvbox
:PROPERTIES:
:header-args:emacs-lisp: :tangle ./themes/cur-gruvbox-theme.el
:END:
