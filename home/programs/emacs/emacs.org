...
This is to check what version of Emacs is being used. I do not test this configuration on any Emacs version less than 29.4. Using this configuration on any version less than 29.4 could lead to unforeseen consequences.
...
Requiring ~use-package~ as that is what I use for almost all of my configuration. It is a macro that allows for easily declaring configuration for a specific [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Named-Features.html][feature]] (think a program or plug-in within Emacs). Please read the [[info:use-package][info manual]] for how to use it and use [[https://jwiegley.github.io/use-package/keywords/][this keyword overview]] for a quick a reference if you need a refresher while using the macro.
...
*** =cur-config-essentials.el= section on tmr
...
** Configurations for Elfeed
:PROPERTIES:
:header-args:emacs-lisp: :tangle ./cur-config/cur-config-elfeed.el
:END:
